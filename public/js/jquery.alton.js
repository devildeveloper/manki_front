...
(function($) {
  ...
  var defaults = {
    ...
    slideNumbersBorderColor: 'rgba(0, 0, 0, 0.1)', // outside color for slide numbers
    slideNumbersColor: '#ff5628', // interior color when slide numbers inactive
        ...
        if (style === 'featuredScroll') {
          ...
          if (!$('.' + settings.firstClass).hasClass('active')) {
            ...
            if (msieversion()) {
              $('.paginate.active').css({
                'margin-left': '-1px',
                'background-color': settings.slideNumbersColor,
                'height': '8px',
                'width': '8px'
              });
            }
